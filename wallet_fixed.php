...
try {
  ...
  out_json(500, ['status'=>'error','message'=>'Server error','detail'=>$e->getMessage()]);
      ...
      $dep = $st->fetch(PDO::FETCH_ASSOC);
      if (!$dep) out_json(404, ['status'=>'error','message'=>'Deposit not found']);
      if ($dep['status'] !== 'initiated' && $dep['status'] !== 'reviewing') out_json(400, ['status'=>'error','message'=>'Deposit already processed']);
      if (empty(<?php
header('Content-Type: application/json; charset=utf-8');
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Headers: Content-Type, Authorization');
...
try {
  ...
  out_json(500, ['status'=>'error','message'=>'Server error','detail'=>$e->getMessage()]);
}FILES['proof']) || !is_uploaded_file(<?php
header('Content-Type: application/json; charset=utf-8');
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Headers: Content-Type, Authorization');
header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
...
try {
  ...
  out_json(500, ['status'=>'error','message'=>'Server error','detail'=>$e->getMessage()]);
}FILES['proof']['tmp_name'])) out_json(400, ['status'=>'error','message'=>'No image uploaded']);
      $dir = __DIR__ . '/uploads/wallet/';
      if (!is_dir($dir)) @mkdir($dir, 0777, true);
      $ext = pathinfo((string)<?php
header('Content-Type: application/json; charset=utf-8');
header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Headers: Content-Type, Authorization');
header('Access-Control-Allow-Methods: GET, POST, OPTIONS');
...
try {
  ...
  out_json(500, ['status'=>'error','message'=>'Server error','detail'=>$e->getMessage()]);
}FILES['proof']['name'], PATHINFO_EXTENSION);
      $ext = $ext ? '.'.preg_replace('/[^a-zA-Z0-9]/', '', $ext) : '';
      $name = uniqid('proof_', true).$ext;
...
try {
  ...
  out_json(500, ['status'=>'error','message'=>'Server error','detail'=>$e->getMessage()]);
}FILES['proof']['tmp_name'], $dir.$name)) out_json(500, ['status'=>'error','message'=>'Failed to save image on server']);
      $proofUrl = base_url($config).'/api/v1/uploads/wallet/'.$name;
      $pdo->prepare("UPDATE wallet_deposits SET proof_url=?, status='reviewing', updated_at=NOW() WHERE id=?")->execute([$proofUrl, $deposit_id]);
      out_json(200, ['status' => 'success', 'message' => 'Screenshot uploaded. Payment under review.']);
    }

...
try {
    ...
    if ($action === 'list') {
        ...
        // Fetch notifications, join sender info and post thumbnail for content types
        $stmt = $pdo->prepare("
            SELECT n.id, n.user_id, n.sender_id, n.type, n.title, n.message, n.reference_id, n.is_read, n.created_at,
                   u.name AS sender_name, u.full_name AS sender_full_name, u.profile_pic AS sender_avatar,
                   CASE WHEN n.type IN ('like','comment','comment_reply','repost','mention')
                        THEN COALESCE(p.file_url, p.media_url, p.thumbnail_url)
                        ELSE NULL END AS reference_image
            FROM notifications n
            LEFT JOIN users u ON u.id = n.sender_id AND n.sender_id > 0
            LEFT JOIN posts p ON p.id = n.reference_id AND n.type IN ('like','comment','comment_reply','repost','mention')
            WHERE n.user_id = ? AND n.type NOT IN ('chat', 'call', 'incoming_call')

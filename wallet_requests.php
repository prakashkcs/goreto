...
$pdo->commit();
      ...
      $typeWord = ucfirst($r["req_type"]);
      send_app_notification($pdo, $uid, 0, "wallet_accept", "$typeWord Approved", "Your {$r['req_type']} request for {$coins} coins has been approved.");
...
$pdo->commit();
      require_once __DIR__ . "/../api/v1/notification_helper.php";
      $typeWord = ucfirst($r["req_type"]);
      send_app_notification($pdo, $uid, 0, "wallet_reject", "$typeWord Rejected", "Your {$r['req_type']} request for {$coins} coins was rejected.");
...
$pdo->commit();
      require_once __DIR__ . "/../api/v1/notification_helper.php";
      send_app_notification($pdo, (int)$deposit["user_id"], 0, "deposit_accept", "QR Deposit Approved", "Your QR deposit for {$deposit['coins']} coins has been approved.");
      $msg = "QR Deposit Approved";
    }

    if ($_POST['qr_action'] === 'reject_qr') {
$pdo->prepare("UPDATE wallet_deposits SET status='rejected', updated_at=NOW() WHERE id=?")->execute([$depositId]);
      require_once __DIR__ . "/../api/v1/notification_helper.php";
      send_app_notification($pdo, (int)$deposit["user_id"], 0, "deposit_reject", "QR Deposit Rejected", "Your QR deposit for {$deposit['coins']} coins was rejected.");
      ...
      <tbody>
        <?php foreach($qrDeposits as $d): ?>
          <tr>
            ...
            <td>
              <?php if (!empty($d['proof_url'])): ?>
                <a class="btn" target="_blank" href="<?= htmlspecialchars($d['proof_url']) ?>">View</a>
              <?php else: ?>

...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'create_plan') {
            ...
            $st = $pdo->prepare("INSERT INTO subscription_plans (creator_id, name, price_coins, duration_days) VALUES (?, ?, ?, ?)");
            $st->execute([$userId, $name, $priceCoins, $durationDays]);
        ...
        if ($action === 'subscribe') {
            ...
            $walletSt = $pdo->prepare("SELECT coins FROM users WHERE id = ?");
            ...
            // Deduct
            $pdo->prepare("UPDATE users SET coins = coins - ? WHERE id = ?")->execute([$price, $userId]);

            // Credit creator
            $pdo->prepare("UPDATE users SET coins = coins + ? WHERE id = ?")->execute([$price, $creatorId]);

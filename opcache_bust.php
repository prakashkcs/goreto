<?php
// One-shot opcache reset + deploy updated files
...
// Deploy updated auth_middleware.php to api/v1/
$apiV1Dir = "$dir/api/v1";
if (!is_dir($apiV1Dir)) {
...
if (empty($token)) {
    ...
    echo json_encode(["status" => "error", "message" => "No auth token provided"]);
    exit;
}
...
if (!$user) {
    http_response_code(401);
    echo json_encode(["status" => "error", "message" => "Token not found in database"]);
    exit;
}
...
// Also invalidate the updated files
$files = [
    __DIR__ . '/db_connect.php',
    __DIR__ . '/profile_v19.php',
    __DIR__ . '/auth.php',
...
// Verify the config path that db_connect.php will use

        ...
        if ($user && !empty($user['fcm_token'])) {
            ...
            if (file_exists($serviceAccountPath)) {
                ...
                if (!empty($projectId)) {
                            ...
                            if ($sDat) {
                                ...
                                if (!empty($sDat['profile_pic'])) {
                                    ...
                                    if (!preg_match('~^https?://~i', $pPic)) {
                                        $fcmPayload['sender_avatar'] = 'https://coinzop.com/ekloadmin/' . ltrim($pPic, '/');
                                    } else {
                    ...
                    // If we have an avatar, set it as the notification image for rich tray display
                    if (!empty($fcmPayload['sender_avatar'])) {
                        $fcmNotification['image'] = $fcmPayload['sender_avatar'];
                    }

                    try {
                        $fcmClient->sendDataMessage($fcmToken, $projectId, $fcmPayload, $fcmNotification, $forceDataOnly);
                    }
                    catch (Throwable $e) {

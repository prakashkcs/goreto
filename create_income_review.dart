...
\$where = "";
if (\$filter === 'pending') \$where = "WHERE m.income_status='pending'";
elseif (\$filter === 'approved') \$where = "WHERE m.income_status='verified'";
elseif (\$filter === 'rejected') \$where = "WHERE m.income_status='none' AND p.id IS NOT NULL";
...
''';
  ...
  File('final_income_review.php').writeAsStringSync(content);
}

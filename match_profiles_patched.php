...
// â”€â”€ SETUP TABLES â”€â”€

try {

    // We add match_profile related columns to the match_profiles table, or just use `users` table

    // Wait, the app calls `api.getMyMatchProfile()` which likely queries something.

    // Let's create `match_profiles` table to store this specific dating info.
...
// â”€â”€ FORCE ADD COLUMNS IF THEY DON'T EXIST â”€â”€
...
try {
    ...
    if ($hasNewProofs || $incomeChanged) {
        $pdo->prepare("DELETE FROM income_proofs WHERE user_id = ? AND status = 'pending'")->execute([$userId]);

...
if ($action === 'update_location' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $nearbyStmt = $pdo->prepare("
        ...
        LIMIT 10
    ");
    $nearbyStmt->execute([$lat, $lng, $lat, $userId, $radiusKm]);
    ...
    if (!empty($nearbyUsers)) {
        ...
        foreach ($nearbyUsers as $nearby) {
            ...
            send_app_notification($pdo, $nearbyUid, $userId, 'nearby', $title, $body);
            ...
            send_app_notification($pdo, $userId, $nearbyUid, 'nearby', $title2, $body2);
...
if ($action === 'nearby' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if (!$userId || !$lat || !$lng) {
        json_out(400, ['status' => 'error', 'message' => 'user_id, lat, lng required']);
    }
    ...
    $stmt = $pdo->prepare("
        ...
        LIMIT ? OFFSET ?
    ");
    $stmt->execute([$lat, $lng, $lat, $userId, $limit, $offset]);
    $profiles = $stmt->fetchAll(PDO::FETCH_ASSOC);

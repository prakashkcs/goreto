...
if ($method === 'GET') {
  ...
  if (in_array('kyc_status', $cols, true)) {
    $select[] = "kyc_status";
  } else {
    $select[] = "'none' AS kyc_status";
  }
  ...
  // Referral code fields — auto-generate if missing
  $u['referral_code'] = '';
  $u['referral_code_edited'] = 0;
  if (in_array('referral_code', $cols, true)) {
    if (!in_array('referral_code_edited', $cols, true)) {
      try { $pdo->exec("ALTER TABLE users ADD COLUMN referral_code_edited TINYINT(1) NOT NULL DEFAULT 0"); } catch (Throwable $_) {}
    }
    ...
    if ($rcRow) {
      $existing = trim((string)($rcRow['referral_code'] ?? ''));
      if ($existing === '') {
        // Auto-generate a unique code based on username + random suffix
        $base = strtoupper(preg_replace('/[^A-Z0-9]/i', '', $u['name'] ?? $u['username'] ?? 'USER'));
        $base = substr($base, 0, 6);
        if (strlen($base) < 4) $base = str_pad($base, 4, 'X');
        do {
          $candidate = $base . strtoupper(substr(bin2hex(random_bytes(2)), 0, 4));
          $dupChk = $pdo->prepare("SELECT COUNT(*) FROM users WHERE referral_code = ?");
          $dupChk->execute([$candidate]);
        } while ((int)$dupChk->fetchColumn() > 0);
        $pdo->prepare("UPDATE users SET referral_code = ? WHERE $colId = ?")->execute([$candidate, $userId]);
        $existing = $candidate;
      }
      $u['referral_code'] = $existing;
      $u['referral_code_edited'] = intval($rcRow['referral_code_edited'] ?? 0);
    }
  }

  // Public relationship partner code
  $u['public_partner'] = null;
  if (table_exists_local($pdo, 'proposals')) {
    $stPartner = $pdo->prepare("
        SELECT 
            u2.name AS partner_name,
            u2.profile_pic AS partner_avatar
        FROM proposals p
        JOIN users u2 ON u2.id = CASE WHEN p.sender_id = :userId THEN p.receiver_id ELSE p.sender_id END
        ...
        LIMIT 1
    ");
    $stPartner->execute(['userId' => (int)$u['id']]);
    $partnerRaw = $stPartner->fetch(PDO::FETCH_ASSOC);
    if ($partnerRaw) {
        $partnerRaw['partner_avatar'] = norm_url($partnerRaw['partner_avatar'] ?? '', $baseUrl);
        $u['public_partner'] = $partnerRaw;
    }
  }
...
if ($method === 'POST') {
  ...
  if (table_exists_local($pdo, 'proposals')) {
    $stPartner = $pdo->prepare("
        ...
        LIMIT 1
    ");
    $stPartner->execute(['userId' => $viewerId]);
    $partnerRaw = $stPartner->fetch(PDO::FETCH_ASSOC);
    if ($partnerRaw) {
        $partnerRaw['partner_avatar'] = norm_url($partnerRaw['partner_avatar'] ?? '', $baseUrl);
        $u['public_partner'] = $partnerRaw;

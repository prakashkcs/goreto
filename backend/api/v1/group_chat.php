...
try {
    ...
    if (empty($config['base_url'])) {
        ...
        $config['base_url'] = $proto . '://' . ($_SERVER['HTTP_HOST'] ?? 'coinzop.com') . '/ekloadmin/api/v1/';
    }
    ...
    if ($action === 'create') {
        ...
        if ($search !== '') {
            ...
            $stmt = $pdo->prepare("SELECT g.id, g.name, g.avatar, g.username, g.bio, g.join_fee, g.monthly_fee, g.is_private,
                ...
                FROM chat_groups g
                WHERE g.id NOT IN (SELECT group_id FROM chat_group_bans WHERE user_id = ?)
                AND g.is_private = 0
        ...
        $stmt = $pdo->prepare("
            SELECT g.id, g.name, g.avatar, g.username, g.bio, g.join_fee, g.monthly_fee, g.is_private, g.message_delay, g.permissions, coalesce(g.created_by,0) as created_by, g.views_count,
            m.role as my_role,
            (SELECT COUNT(*) FROM chat_group_members WHERE group_id = g.id) as member_count,
            1 as is_member,
            (SELECT message    FROM chat_group_messages WHERE group_id = g.id ORDER BY id DESC LIMIT 1) as last_message,
            (SELECT type       FROM chat_group_messages WHERE group_id = g.id ORDER BY id DESC LIMIT 1) as last_message_type,
            (SELECT sender_id  FROM chat_group_messages WHERE group_id = g.id ORDER BY id DESC LIMIT 1) as last_message_sender_id,
            (SELECT u.name FROM chat_group_messages cgm JOIN users u ON u.id = cgm.sender_id WHERE cgm.group_id = g.id ORDER BY cgm.id DESC LIMIT 1) as last_message_sender,
        ...
        $st = $pdo->prepare("
            SELECT m.id, m.sender_id, m.message, m.type, m.voice_duration, m.created_at,
        ...
        $groupId = (int) ($input['group_id'] ?? 0);
        $username = trim($input['username'] ?? '');
        $inviteUserId = (int) ($input['user_id'] ?? 0);
        requireGroupAdmin($pdo, $groupId, $userId);

        if ($inviteUserId > 0) {
            $st = $pdo->prepare("SELECT id, name FROM users WHERE id = ?");
            $st->execute([$inviteUserId]);
        } else {
        ...
        ");
        $st->execute([$groupId]);
        out_json(200, ['status' => 'success', 'my_role' => $myRole, 'members' => $st->fetchAll(PDO::FETCH_ASSOC)]);
    } elseif ($action === 'send_media') {
        $groupId = (int) ($_POST['group_id'] ?? 0);
        $type = $_POST['type'] ?? 'image';
        $voiceDuration = (int) ($_POST['voice_duration'] ?? 0);

        // Ensure voice_duration column exists
        try { $pdo->exec("ALTER TABLE chat_group_messages ADD COLUMN voice_duration INT DEFAULT 0"); } catch (Exception $e) {}
        ...
        if (move_uploaded_file($_FILES['media']['tmp_name'], $uploadsDir . '/' . $filename)) {
            ...
            $pdo->prepare("INSERT INTO chat_group_messages (group_id, sender_id, message, type, voice_duration) VALUES (?, ?, ?, ?, ?)")
                ->execute([$groupId, $userId, $mediaUrl, $type, $voiceDuration]);
        ...
        foreach ($viewers as &$v) {
            if (!empty($v['avatar']) && strpos($v['avatar'], 'http') !== 0) {
                $v['avatar'] = $config['base_url'] . $v['avatar'];
            }
        }

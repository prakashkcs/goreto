...
try {
    ...
    if ($action === 'get_live_users') {
        ...
        $stmt = $pdo->query("
            SELECT ls.user_id, ls.user_name, ls.viewer_count, ls.started_at,
                   ...
                   COALESCE(u.name, u.username, ls.user_name) AS display_name
    ...
    // ── INVITE TO LIVE ──────────────────────────────────────────────────────
    if ($action === 'invite_to_live') {
        require_once __DIR__ . '/notification_helper.php';
        $input   = json_decode(file_get_contents('php://input'), true) ?: $_POST;
        $rawIds  = $input['user_ids'] ?? $_REQUEST['user_ids'] ?? '';
        $userIds = array_filter(array_map('intval',
            is_array($rawIds) ? $rawIds : explode(',', (string)$rawIds)));
        $sent = 0;
        foreach ($userIds as $inviteeId) {
            if ($inviteeId <= 0 || $inviteeId === $userId) continue;
            send_app_notification(
                $pdo, $inviteeId, $userId,
                'live_invite',
                "$name is live now!",
                "Tap to join the live stream",
                $userId

...
class PushNotificationFCM
{
    ...
    {
        // Cache token in a temp file — valid for 3600 s, refresh 60 s early.
        $cacheFile = sys_get_temp_dir() . '/goreto_fcm_token.json';
    ...
    {
        // NOTE: do NOT put 'title' or 'body' here. OneSignal's FCMBroadcastReceiver
        // (bundled via the onesignal_flutter plugin) inspects every incoming
        // data message and, if it sees those keys, posts its own heads-up
        // notification — on top of the native IncomingCallActivity we already
        // show. The native handler builds its own caller-named notification
    ...
    public function sendDataMessage($fcmToken, $projectId, $dataPayload, $notificationPayload = null, $forceDataOnly = true)
    {
        ...
        if ($notificationPayload && !$forceDataOnly) {
            // iOS ONLY: deliver the alert via APNS so it displays in the
            // background (data-only does not show on iOS). Deliberately DO NOT
            // set message.notification or android.notification: on Android the
            // native CallFirebaseMessagingService renders the tray notification
            // from the data payload, and a notification block here makes Android
            // auto-render a SECOND (black & white) duplicate.
    ...
    public function sendChatMessage($fcmToken, $projectId, $senderName, $messageText, $senderId, $senderAvatar = '')
    {
        $data = [
            'action'        => 'chat',
            'type'          => 'chat',
        ...
        // Data-only: background handler creates channel and shows tray manually.
        // More reliable on OEM devices (Xiaomi, Oppo, etc.) with aggressive battery optimization.
        return $this->sendDataMessage($fcmToken, $projectId, $data, null, true);
    }

    public function sendFollowNotification($fcmToken, $projectId, $followerName, $followerId, $followerAvatar = '')
    {
        $data = [
            'action'        => 'follow',
            'type'          => 'follow',
    ...
    public function sendProposalNotification($fcmToken, $projectId, $proposerName, $proposerId, $proposerAvatar = '')
    {
        $data = [
            'action'        => 'proposal',
            'type'          => 'proposal',
            'sender_id'     => (string)$proposerId,
            'sender_name'   => $proposerName,
            'sender_avatar' => $proposerAvatar,
            'title'         => $proposerName,
            'body'          => 'sent you a proposal 💌',
        ];
        return $this->sendDataMessage($fcmToken, $projectId, $data, null, true);
    }

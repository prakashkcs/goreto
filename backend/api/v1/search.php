...
try {
    ...
    if ($action === 'users') {
        $stmt = $pdo->prepare("SELECT id, name, username, profile_pic as avatar FROM users WHERE (name LIKE ? OR username LIKE ?) AND COALESCE(privacy_allow_find_id, 1) = 1 AND is_banned = 0 $blockFilterUsers ORDER BY (username LIKE ?) DESC, (name LIKE ?) DESC LIMIT 50");
        $paramsUsers[] = "$query%";
        $paramsUsers[] = "$query%";

...
try {
  ...
  if ($action === 'update_fcm_token' || $action === 'manual_log') {
    ...
    $newToken = $fcmToken !== '' ? $fcmToken : null;
    // Remove this token from any other account so only one account receives notifications per device
    if ($newToken !== null) {
        $db->prepare('UPDATE users SET fcm_token = NULL WHERE fcm_token = ? AND id != ?')->execute([$newToken, $user['id']]);
    }
    $db->prepare('UPDATE users SET fcm_token = ? WHERE id = ?')->execute([$newToken, $user['id']]);
    if ($fcmToken === '') {
        $db->prepare('UPDATE users SET latitude = NULL, longitude = NULL, location_updated_at = NULL WHERE id = ?')->execute([$user['id']]);
    }
...
if ($action === 'sessions') {
    ...
    $tok  = trim(str_replace('Bearer ', '', $hdrs['Authorization'] ?? $hdrs['authorization'] ?? ''));
    $meSt = $db->prepare('SELECT user_id FROM user_auth_tokens WHERE token = ? AND revoked_at IS NULL LIMIT 1');
    $meSt->execute([$tok]);
    $me = (int)($meSt->fetchColumn() ?: 0);
    if (!$me) { http_response_code(401); echo json_encode(['status' => false, 'message' => 'Unauthorized']); exit;; }

    $currentToken = '';
    $headers = function_exists('getallheaders') ? getallheaders() : [];
    $auth = $headers['Authorization'] ?? $headers['authorization'] ?? '';
    if (stripos($auth, 'Bearer ') === 0) $currentToken = trim(substr($auth, 7));

    $st = $db->prepare("
        SELECT id, device_name, platform, app_version, user_agent, ip_address,
               created_at, last_used_at, token
    ...
    $sessions = array_map(function($r) use ($currentToken) {
        $device = $r['device_name'] ?: ($r['platform'] ?: 'Unknown device');
        if (!empty($r['platform']) && !str_contains($device, $r['platform'])) {
            $device .= ' · ' . $r['platform'];
        }
        return [
            'id'          => $r['id'],
            'device'      => $device,
            'ip'          => $r['ip_address'] ?? '',
            'last_active' => $r['last_used_at'] ?? $r['created_at'] ?? '',
            'is_current'  => ($r['token'] === $currentToken),
        ];
    }, $rows);

    http_response_code(200); echo json_encode(['status' => true, 'sessions' => $sessions]); exit;;
}
...
if ($action === 'terminate_session') {
    ...
    if ($sessionId <= 0) { http_response_code(400); echo json_encode(['status' => false, 'message' => 'session_id required']); exit;; }

    $st = $db->prepare("UPDATE user_auth_tokens SET revoked_at = NOW() WHERE id = ? AND user_id = ?");
    $st->execute([$sessionId, $me]);
    if ($st->rowCount() > 0) {
        http_response_code(200); echo json_encode(['status' => true, 'message' => 'Session terminated']); exit;;
    } else {
        http_response_code(404); echo json_encode(['status' => false, 'message' => 'Session not found']); exit;;
    }
}
...
if ($action === 'logout_all') {
    ...
    $st = $db->prepare("UPDATE user_auth_tokens SET revoked_at = NOW() WHERE user_id = ? AND token != ? AND revoked_at IS NULL");
    $st->execute([$me, $currentToken]);
    // Clear stale location so this account no longer appears in nearby results
    $db->prepare('UPDATE users SET latitude = NULL, longitude = NULL, location_updated_at = NULL WHERE id = ?')->execute([$me]);
    http_response_code(200); echo json_encode(['status' => true, 'revoked' => $st->rowCount()]); exit;;
}

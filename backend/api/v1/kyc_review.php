...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verification_id'], $_POST['action'])) {
  ...
  try {
    ...
    if ($action === 'approve') {
      $pdo->prepare("UPDATE kyc_verifications SET status='verified' WHERE id=?")
        ->execute([$sid]);

      $pdo->prepare("UPDATE users SET kyc_status='verified', subscription_status='active' WHERE id=?")
        ->execute([$uid]);

      // Sync with user_kyc table used by some other parts of the app
...
<div class="section">
  <div class="head">
    ...
    <div class="search">
      ...
      <a class="btn" href="?filter=verified">Verified</a>
          ...
          <?php foreach ($rows as $r): ?>
            <tr>
              ...
              <td>
                <?php
  $st = (string)$r['status'];
  $cls = ($st === 'pending') ? 'warn' : (($st === 'verified' || $st === 'approved') ? '' : 'danger');
?>

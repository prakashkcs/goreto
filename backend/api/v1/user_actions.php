...
// Parse JSON body into $_POST when client sends application/json (e.g. Dio default)
if (empty($_POST)) {
    $_rawInput = file_get_contents('php://input');
    if ($_rawInput) {
        $_decoded = json_decode($_rawInput, true);
        if (is_array($_decoded)) {
            $_POST = array_merge($_POST, $_decoded);
        }
    }
}
...
function insertNotification($pdo, $userId, $fromUserId, $type, $message, $title = null)
{
    // Notifications table requires sender_id (not from_user_id) and title NOT NULL.
    // Derive a sensible title from the type when caller didn't pass one.
    if ($title === null || $title === '') {
        $titleMap = [
            'proposal' => 'New Proposal',
            'proposal_accepted' => 'Proposal Accepted',
            'proposal_match' => 'It is a Match',
            'follow' => 'New Follower',
            'like' => 'New Like',
            'comment' => 'New Comment',
            'gift' => 'New Gift',
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, 0, NOW())
        ");
        $stmt->execute([$userId, $fromUserId, $type, $title, $message]);
    } catch (Throwable $e) {
        error_log('insertNotification failed: ' . $e->getMessage());
    }
}
...
try {
    ...
    $action = $_GET['action'] ?? $_REQUEST['action'] ?? $_POST['action'] ?? '';
    ...
    if ($action === 'send_proposal') {
        ...
        // ── RATE LIMIT CHECKS ──
        // 1-hour rate limit: max 10 proposals per hour
        $rlStmt1 = $pdo->prepare("SELECT COUNT(*) FROM proposals WHERE sender_id = ? AND created_at >= NOW() - INTERVAL 1 HOUR");
        $rlStmt1->execute([$userId]);
        if ((int) $rlStmt1->fetchColumn() >= 10) {
            echo json_encode(['status' => 'error', 'message' => "You're sending proposals too fast. Please wait a while before sending more.", 'error_code' => 'rate_limited']);
            exit;
        }

        // 24-hour rate limit: max 30 proposals per day
        $rlStmt2 = $pdo->prepare("SELECT COUNT(*) FROM proposals WHERE sender_id = ? AND created_at >= NOW() - INTERVAL 24 HOUR");
        $rlStmt2->execute([$userId]);
        if ((int) $rlStmt2->fetchColumn() >= 30) {
            echo json_encode(['status' => 'error', 'message' => "You've reached today's proposal limit. Try again tomorrow.", 'error_code' => 'daily_limit']);
            exit;
        }
        ...
        // fetch sender avatar for rich notification
        $_avatarRow = $pdo->prepare('SELECT profile_pic FROM users WHERE id=?');
        $_avatarRow->execute([$userId]);
        $_senderAvatar = $_avatarRow->fetchColumn() ?: '';
        sendFcmNotification($pdo, $targetId, $senderName, 'sent you a proposal 🌹', [
            'action'        => 'new_proposal',
            'type'          => 'proposal',
    ...
    elseif ($action === 'get_proposals') {
        ...
        if ($type === 'received') {
            $stmt = $pdo->prepare("
                SELECT p.id AS proposal_id, p.sender_id, p.status, p.is_read, p.created_at,
                       u.name, u.full_name, u.profile_pic AS avatar, u.age, u.location AS city
                FROM proposals p
                JOIN users u ON u.id = p.sender_id
                WHERE p.receiver_id = ? AND p.status = 'pending' AND NOT EXISTS(SELECT 1 FROM proposals cc WHERE cc.status='accepted' AND ((cc.sender_id=p.sender_id AND cc.receiver_id=p.receiver_id) OR (cc.sender_id=p.receiver_id AND cc.receiver_id=p.sender_id)))
            ...
            $stmt = $pdo->prepare("
                SELECT p.id AS proposal_id, p.receiver_id, p.status, p.is_read, p.created_at,
                       u.name, u.full_name, u.profile_pic AS avatar, u.age, u.location AS city
                FROM proposals p
                JOIN users u ON u.id = p.receiver_id
                WHERE p.sender_id = ? AND p.status = 'pending' AND NOT EXISTS(SELECT 1 FROM proposals cc WHERE cc.status='accepted' AND ((cc.sender_id=p.sender_id AND cc.receiver_id=p.receiver_id) OR (cc.sender_id=p.receiver_id AND cc.receiver_id=p.sender_id)))
                ORDER BY p.created_at DESC
                LIMIT 100
            ");
            $stmt->execute([$userId]);
    ...
    elseif ($action === 'accept_terms') {
        $pdo->prepare("UPDATE users SET terms_accepted=1, terms_accepted_at=NOW() WHERE id=?")->execute([$userId]);
        echo json_encode(['status' => 'success']);
    }
    elseif ($action === 'get_terms_status') {
        $st = $pdo->prepare("SELECT terms_accepted FROM users WHERE id=? LIMIT 1");
        $st->execute([$userId]);
        echo json_encode(['status' => 'success', 'terms_accepted' => (int) $st->fetchColumn()]);
    }
    ...
    elseif ($action === 'disconnect') {
        ...
        $videoUrl = null;
        $videoField = null;
        if (!empty($_FILES['liveness_video']['tmp_name']) && is_uploaded_file($_FILES['liveness_video']['tmp_name'])) {
            $videoField = 'liveness_video';
        } elseif (!empty($_FILES['video']['tmp_name']) && is_uploaded_file($_FILES['video']['tmp_name'])) {
            $videoField = 'video';
        }
        if ($videoField) {
            $dir = __DIR__ . '/uploads/kyc/';
            if (!is_dir($dir)) @mkdir($dir, 0777, true);
            $ext = pathinfo((string)$_FILES[$videoField]['name'], PATHINFO_EXTENSION);
            $ext = $ext ? '.' . preg_replace('/[^a-zA-Z0-9]/', '', $ext) : '.mp4';
            $fname = uniqid('kyc_', true) . $ext;
            if (move_uploaded_file($_FILES[$videoField]['tmp_name'], $dir . $fname)) {
                $videoUrl = 'https://goreto.org/ekloadmin/api/v1/uploads/kyc/' . $fname;
            }
        }
        if (!$videoUrl) {
            echo json_encode(['status' => 'error', 'message' => 'Video file required']);
            exit;
        }

        $savedUrls = [];
        foreach (['id_front', 'id_back', 'selfie'] as $imgField) {
            if (!empty($_FILES[$imgField]['tmp_name']) && is_uploaded_file($_FILES[$imgField]['tmp_name'])) {
                $dir = __DIR__ . '/uploads/kyc/';
                if (!is_dir($dir)) @mkdir($dir, 0777, true);
                $ext2 = pathinfo((string)$_FILES[$imgField]['name'], PATHINFO_EXTENSION);
                $ext2 = $ext2 ? '.' . preg_replace('/[^a-zA-Z0-9]/', '', $ext2) : '.jpg';
                $fname2 = uniqid('kyc_' . $imgField . '_', true) . $ext2;
                if (move_uploaded_file($_FILES[$imgField]['tmp_name'], $dir . $fname2)) {
                    $savedUrls[$imgField] = 'https://goreto.org/ekloadmin/api/v1/uploads/kyc/' . $fname2;
                }
            }
        }

        $pdo->prepare("INSERT INTO kyc_submissions (user_id, level, task_id, full_name, video_url, id_front, id_back, selfie_pic, status) VALUES (?,?,?,?,?,?,?,?,'pending')")->execute([$userId, $level, $taskId ?: null, $fullName, $videoUrl, $savedUrls['id_front'] ?? null, $savedUrls['id_back'] ?? null, $savedUrls['selfie'] ?? null]);
        ...
        if ($level === 'basic') {
            $pdo->prepare("UPDATE user_kyc SET basic_status='pending', full_name=?, basic_video_url=?, basic_task_id=?, basic_submitted_at=NOW() WHERE user_id=?")
                ->execute([$fullName, $videoUrl, $taskId ?: null, $userId]);
        } else {
            $pdo->prepare("UPDATE user_kyc SET full_status='pending', full_video_url=?, full_task_id=?, full_submitted_at=NOW() WHERE user_id=?")
                ->execute([$videoUrl, $taskId ?: null, $userId]);
        }
        echo json_encode(['status' => 'success', 'message' => ucfirst($level) . ' KYC submitted successfully']);
        // ── CANCEL KYC ──

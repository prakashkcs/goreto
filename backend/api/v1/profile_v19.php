...
function getProfile($userId)
{
    ...
    try {
        ...
        foreach (['bio', 'location', 'profile_pic', 'cover_pic', 'total_proposals', 'rating', 'kyc_status', 'subscription_status', 'subscriber_only_dm', 'pay_per_min_enabled', 'pay_per_min_rate', 'ppm_charge_friends', 'privacy_show_name_random_call', 'privacy_direct_random_call', 'privacy_share_distance', 'cross_gender_proposal'] as $col) {
        ...
        // Override kyc_status with the canonical value from user_kyc.
        // The legacy users.kyc_status column is no longer updated on submit
        // and can return stale values like 'rejected' after a new pending
        // submission, leaking into ProfileService.cacheProfile which then
        // stomps the correct value KycScreen wrote to SharedPreferences.
        if ($user) {
            try {
                $ukSt = $pdo->prepare("SELECT basic_status FROM user_kyc WHERE user_id = ? LIMIT 1");
                $ukSt->execute([$userId]);
                $ukRow = $ukSt->fetch(PDO::FETCH_ASSOC);
                if ($ukRow && !empty($ukRow['basic_status'])) {
                    $user['kyc_status'] = $ukRow['basic_status'] === 'approved'
                        ? 'verified'
                        : $ukRow['basic_status'];
                }
            } catch (Throwable $e) { /* fall back to legacy column */ }
        }
        ...
        // is_followed_by = does the viewed user follow the viewer? Drives the
        // 'Follow Back' label on the profile screen when they follow you but
        ...
        if (!empty($token)) {
            ...
            if ($meId) {
                ...
                $stB = $pdo->prepare("SELECT 1 FROM follows WHERE follower_id = ? AND following_id = ? LIMIT 1");
                $stB->execute([$userId, $meId]);
                if ($stB->fetchColumn()) {
                    $user['is_followed_by'] = true;
                }
                // Mutual friends: people both viewer and profile owner follow
                $user['mutual_friends'] = [];
                $user['mutual_count']   = 0;
                if ((int)$meId !== (int)$userId) {
                    $mfSt = $pdo->prepare("
                        SELECT u.id, u.name, u.username, COALESCE(u.profile_pic,'') AS profile_pic
                        FROM follows fa
                        JOIN follows fb ON fb.following_id = fa.following_id
                        JOIN users u ON u.id = fa.following_id
                        WHERE fa.follower_id = ? AND fb.follower_id = ?
                          AND fa.following_id != ? AND fa.following_id != ?
                          AND u.is_banned = 0
                        ORDER BY u.id DESC
                        LIMIT 6
                    ");
                    $mfSt->execute([$meId, $userId, (int)$meId, (int)$userId]);
                    $mfRows = $mfSt->fetchAll(PDO::FETCH_ASSOC);
                    $base = 'https://goreto.org/ekloadmin/';
                    $mf = array_map(function($r) use ($base) {
                        $av = (string)($r['profile_pic'] ?? '');
                    ...
                    if (count($mfRows) == 6) {
                        $cntSt = $pdo->prepare("SELECT COUNT(*) FROM follows fa JOIN follows fb ON fb.following_id=fa.following_id WHERE fa.follower_id=? AND fb.follower_id=? AND fa.following_id!=? AND fa.following_id!=?");
                        $cntSt->execute([$meId, $userId, (int)$meId, (int)$userId]);
                        $user['mutual_count'] = (int)$cntSt->fetchColumn();
                        $mf = array_slice($mf, 0, 5);
                    } else {
                        $user['mutual_count'] = count($mfRows);
                    }
                    $user['mutual_friends'] = $mf;
                }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        // ── Ensure age + consent columns exist (guarded ALTER) ──
        try { if (!in_array('dob', $dbCols, true)) { $pdo->exec("ALTER TABLE users ADD COLUMN dob DATE NULL DEFAULT NULL"); $dbCols[] = 'dob'; } } catch (Throwable $e) {}
        try { if (!in_array('age', $dbCols, true)) { $pdo->exec("ALTER TABLE users ADD COLUMN age INT NULL DEFAULT NULL"); $dbCols[] = 'age'; } } catch (Throwable $e) {}
        try { if (!in_array('cross_border_consent', $dbCols, true)) { $pdo->exec("ALTER TABLE users ADD COLUMN cross_border_consent TINYINT(1) NOT NULL DEFAULT 0"); $dbCols[] = 'cross_border_consent'; } } catch (Throwable $e) {}

        // ── 18+ age gate: derive dob/age and reject minors ──
        if (isset($body['date_of_birth']) && $body['date_of_birth'] !== '') {
            $body['dob'] = $body['date_of_birth'];
        }
        if (isset($body['dob']) && $body['dob'] !== '') {
            $ts = strtotime((string) $body['dob']);
            if ($ts === false) {
            ...
            if ($age < 18) {
                echo json_encode(['status' => 'error', 'message' => 'You must be at least 18 years old to use this app.', 'code' => 'underage']);
                exit;
            }
            $body['age'] = $age;
        }
        ...
        $allowed = array_filter(['name', 'bio', 'location', 'gender', 'age', 'dob', 'is_match_visible', 'latitude', 'longitude', 'profile_pic', $coverCol]);
        $sets = [];
        $vals = [];

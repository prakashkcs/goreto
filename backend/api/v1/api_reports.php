...
$colsToAdd = [
    ...
    'is_csae'     => "TINYINT(1) NOT NULL DEFAULT 0 AFTER `status`",
];
...
if ($method === 'POST' && $action === 'submit_report') {
    ...
    // ── Insert report ─────────────────────────────────────────────────────────
    try {
        $isCsae = (stripos($reason, 'csae') !== false || stripos($reason, 'child sexual') !== false || stripos($reason, 'child abuse') !== false || stripos($reason, 'exploitation') !== false) ? 1 : 0;
        $stmt = $pdo->prepare("
            INSERT INTO `user_reports`
                (reporter_id, reported_id, report_type, post_id, reason, details, image_url, is_csae, status, created_at, updated_at)
            VALUES
                (?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW(), NOW())
        ");
        $stmt->execute([

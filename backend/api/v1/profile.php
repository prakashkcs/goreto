...
$baseUrl = 'https://coinzop.com/ekloadmin';
...
if ($method === 'GET') {
  ...
  // Block banned profiles from being viewed
  if (!empty($u['is_banned']) && (int)$u['is_banned'] === 1) {
      // Only block if viewing someone else's profile
      $viewerUser = null;
      try { $viewerUser = requireUser($pdo); } catch(Throwable $_) {}
      if (!$viewerUser || (int)$viewerUser['id'] !== (int)$u['id']) {
          echo json_encode(['status' => false, 'message' => 'User not found']);
          exit;
      }
  }

  // Block deactivated accounts from other users
  try {
    $dq = $pdo->prepare("SELECT IFNULL(deactivated_type,'') AS dt, deactivated_until FROM users WHERE id=? LIMIT 1");
    $dq->execute([$userId]);
    $drow = $dq->fetch(PDO::FETCH_ASSOC);
    if (!empty($drow['dt'])) {
      $dIsDeact = true;
      if ($drow['dt'] !== 'login' && !empty($drow['deactivated_until']) && strtotime($drow['deactivated_until']) < time()) {
        $dIsDeact = false; // period expired
      }
      if ($dIsDeact && $vid !== $userId) {
        echo json_encode(['status' => false, 'message' => 'This account is temporarily unavailable']);
        exit;
      }
    }
  } catch (Throwable $_) {}
  ...
  // Daily streak — shown to all viewers. Counts only if active today/yesterday.
  try {
    $skSt = $pdo->prepare("SELECT current_streak, longest_streak, last_active_date FROM user_streaks WHERE user_id=? LIMIT 1");
    $skSt->execute([(int)$u['id']]);
    $sk = $skSt->fetch(PDO::FETCH_ASSOC);
    $cs = (int)($sk['current_streak'] ?? 0);
    $la = $sk['last_active_date'] ?? null;
    $disp = ($la === date('Y-m-d') || $la === date('Y-m-d', strtotime('-1 day'))) ? $cs : 0;
    $u['streak'] = $disp;
    $u['current_streak'] = $disp;
    $u['longest_streak'] = (int)($sk['longest_streak'] ?? 0);
    // Seconds until the daily streak would break (for the app countdown).
    $skBreak = $la ? (strtotime($la . ' 00:00:00 UTC') + 2 * 86400) : 0;
    $u['streak_seconds_left'] = ($skBreak > time()) ? ($skBreak - time()) : 0;
  } catch (Throwable $_) { $u['streak'] = 0; $u['current_streak'] = 0; $u['longest_streak'] = 0; }

  // Chat streak — shown only once it has 'started' (>= admin threshold) and is alive.
  try {
    $thr = (int)($pdo->query("SELECT chat_start_threshold FROM streak_settings WHERE id=1")->fetchColumn() ?: 3);
    $csSt = $pdo->prepare("SELECT current_streak, last_active_date FROM user_chat_streaks WHERE user_id=? LIMIT 1");
    $csSt->execute([(int)$u['id']]);
    $cs = $csSt->fetch(PDO::FETCH_ASSOC);
    $ccur = (int)($cs['current_streak'] ?? 0);
    $cla = $cs['last_active_date'] ?? null;
    $calive = ($cla === date('Y-m-d') || $cla === date('Y-m-d', strtotime('-1 day')));
    $u['chat_streak'] = ($calive && $ccur >= $thr) ? $ccur : 0;
  } catch (Throwable $_) { $u['chat_streak'] = 0; }

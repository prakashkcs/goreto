...
function requireUser(PDO $pdo): array
{
    ...
    // Deactivation fields (temporary deactivate)
    $deactEnabled = has_col($pdo, 'deactivated_until') && has_col($pdo, 'deactivated_type');
    if ($deactEnabled) {
        $select[] = 'u.deactivated_until';
        $select[] = 'u.deactivated_type';
    } else {
        $select[] = 'NULL AS deactivated_until';
        $select[] = 'NULL AS deactivated_type';
    }
    ...
    // Deactivation: auto-reactivate if timed period has passed
    try {
        $deactType  = $user['deactivated_type']  ?? null;
        $deactUntil = $user['deactivated_until'] ?? null;
        if ($deactType !== null && $deactType !== 'login' && $deactUntil !== null) {
            $now = new DateTime('now');
            $until = new DateTime((string)$deactUntil);
            if ($until <= $now) {
                // Period expired — silently reactivate
                $pdo->prepare('UPDATE users SET deactivated_until=NULL, deactivated_type=NULL WHERE id=?')
                    ->execute([$user['id']]);
                $user['deactivated_until'] = null;
                $user['deactivated_type']  = null;
            }
        }
    } catch (Throwable $e) {}

...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // ── cancel (subscriber-side) or terminate (creator-side) ──
        if ($action === 'cancel' || $action === 'cancel_subscription' || $action === 'terminate_subscriber') {
            $subId = intval($body['subscription_id'] ?? $body['id'] ?? 0);
            if ($subId <= 0)
                out_json(400, ['status' => 'error', 'message' => 'subscription_id required']);

            // Allow either the subscriber themselves OR the creator of the
            // plan (so creators can terminate a misbehaving subscriber).
            $chk = $pdo->prepare("SELECT * FROM user_subscriptions WHERE id = ? AND (subscriber_id = ? OR creator_id = ?)");
            $chk->execute([$subId, $userId, $userId]);
            $row = $chk->fetch(PDO::FETCH_ASSOC);
            if (!$row)
                out_json(403, ['status' => 'error', 'message' => 'Not your subscription']);

            $isCreatorTerminate = ((int)$row['creator_id'] === (int)$userId) &&
                ((int)$row['subscriber_id'] !== (int)$userId);

            $newStatus = $isCreatorTerminate ? 'terminated' : 'cancelled';
            $pdo->prepare("UPDATE user_subscriptions SET status = ? WHERE id = ?")
                ->execute([$newStatus, $subId]);
            out_json(200, [
                'status' => 'success',
                'message' => $isCreatorTerminate

...
if ($action === 'update_location' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Accept lat/lng from: JSON body, form POST, or query string
    $raw = file_get_contents('php://input');
    $body = [];
    if (!empty($raw)) {
        $json = json_decode($raw, true);
        if (is_array($json)) $body = $json;
        else parse_str($raw, $body);  // form-encoded body
    }
    // Merge with $_POST and $_GET as fallback
    $body = array_merge($_GET, $_POST, $body);
    $lat = isset($body['lat']) && $body['lat'] !== '' ? floatval($body['lat']) : null;
    $lng = isset($body['lng']) && $body['lng'] !== '' ? floatval($body['lng']) : null;
    ...
    $stmt = $pdo->prepare("INSERT INTO match_profiles (user_id, lat, lng, last_location_update)
        VALUES (?, ?, ?, UTC_TIMESTAMP())
        ON DUPLICATE KEY UPDATE lat = VALUES(lat), lng = VALUES(lng), last_location_update = UTC_TIMESTAMP()");
    ...
    $nearbyStmt = $pdo->prepare("
        SELECT mp.user_id, mp.lat, mp.lng, mp.last_nearby_notif,
               ...
               COALESCE(u.privacy_share_distance, 0) AS share_distance,
               (6371 * acos(
                   cos(radians(?)) * cos(radians(mp.lat)) *
                   cos(radians(mp.lng) - radians(?)) +
                   sin(radians(?)) * sin(radians(mp.lat))
               )) AS distance_km,
               TIMESTAMPDIFF(SECOND, mp.last_nearby_notif, UTC_TIMESTAMP()) AS sec_since_notif
        ...
        WHERE mp.user_id != ?
          ...
          AND COALESCE(u.privacy_nearby_alert, 1) = 1
          AND mp.lat IS NOT NULL
          AND mp.lng IS NOT NULL
          AND mp.last_location_update >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL 24 HOUR)
          AND NOT EXISTS (
          ...
          AND (
              SELECT COALESCE(LOWER(m2.gender),'')
              FROM match_profiles m2 WHERE m2.user_id = ?
          ) != LOWER(COALESCE(mp.gender,''))
          AND LOWER(COALESCE(mp.gender,'')) != ''
        HAVING distance_km <= ?
        ORDER BY distance_km ASC
        LIMIT 10
    ");
    $nearbyStmt->execute([$lat, $lng, $lat, $userId, $userId, $userId, $userId, $userId, $userId, $radiusKm]);
    ...
    if (!empty($nearbyUsers)) {
        ...
        $myNotifStmt = $pdo->prepare("SELECT TIMESTAMPDIFF(SECOND, last_nearby_notif, UTC_TIMESTAMP()) as my_sec FROM match_profiles WHERE user_id = ?");
        ...
        // Also check current user's share_distance for notifying others about them
        $myShareDistStmt = $pdo->prepare("SELECT COALESCE(privacy_share_distance, 1) AS share_dist FROM users WHERE id = ?");
        $myShareDistStmt->execute([$userId]);
        $myShareDist = (int)(($myShareDistStmt->fetch(PDO::FETCH_ASSOC))['share_dist'] ?? 1);

        foreach ($nearbyUsers as $nearby) {
            $nearbyUid = (int)$nearby['user_id'];
            $nearbySharesDist = !empty($nearby['share_distance']);
            ...
            if ($secSince !== null && $secSince < 300) {
                continue;
            }

            // Notify the nearby user about the current user approaching them
            $title = '📍 Someone is nearby!';
            $body = $myShareDist
                ? "$myName is about {$distMeters}m away from you right now."
                : "$myName is nearby you right now.";
            send_app_notification($pdo, $nearbyUid, $userId, 'nearby', $title, $body, null, false, [
                    'distance' => ($myShareDist ? ($distMeters . 'm') : ''),
                    'sender_distance' => ($myShareDist ? ($distMeters . 'm') : ''),
                ]);

            // Notify the current user about the nearby person
            $theirName = $nearby['name'] ?: $nearby['username'] ?: 'Someone';
            $title2 = '📍 Someone is nearby!';
            $body2 = $nearbySharesDist
                ? "$theirName is about {$distMeters}m away from you right now."
                : "$theirName is nearby you right now.";
            send_app_notification($pdo, $userId, $nearbyUid, 'nearby', $title2, $body2, null, false, [
                    'distance' => ($nearbySharesDist ? ($distMeters . 'm') : ''),
                    'sender_distance' => ($nearbySharesDist ? ($distMeters . 'm') : ''),
                ]);

            // Update cooldown for both
            $pdo->prepare("UPDATE match_profiles SET last_nearby_notif = UTC_TIMESTAMP() WHERE user_id = ?")->execute([$nearbyUid]);
        }
        // Update cooldown for current user
        $pdo->prepare("UPDATE match_profiles SET last_nearby_notif = UTC_TIMESTAMP() WHERE user_id = ?")->execute([$userId]);
    }
...
if ($action === 'feed' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $stmt = $pdo->prepare("
        ...
        FROM match_profiles mp
        JOIN users u ON u.id = mp.user_id
        WHERE mp.user_id != ? AND mp.is_visible = 1 AND u.is_banned = 0
          AND NOT EXISTS (
              SELECT 1 FROM user_blocks b
              WHERE (b.blocker_id = ? AND b.blocked_id = mp.user_id)
                 OR (b.blocker_id = mp.user_id AND b.blocked_id = ?)
...
if ($action === 'nearby' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($hasLocation) {
        $sql = "
            SELECT mp.*, u.name, u.username, u.profile_pic as user_avatar,
                   COALESCE(u.privacy_share_distance, 0) AS _share_dist,
                   (6371 * acos(
                       cos(radians(?)) * cos(radians(mp.lat)) *
                       cos(radians(mp.lng) - radians(?)) +
                       sin(radians(?)) * sin(radians(mp.lat))
    ...
    if ($hasLocation) {
        ...
        $sql .= " ORDER BY mp.rating DESC, mp.last_location_update DESC, mp.user_id DESC";
    }

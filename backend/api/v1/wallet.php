...
function base_url($config): string {
  return rtrim(($config['base_url'] ?? 'https://goreto.org/ekloadmin'), '/');
}
...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'transactions') {
      ...
      // Collect user IDs referenced as "user {id}" in notes and chat session IDs
      $userIds = [];
      $sessionIds = [];
      foreach ($rows as $row) {
        if (preg_match_all('/\buser (\d+)\b/i', (string)($row['note'] ?? ''), $m)) {
          foreach ($m[1] as $uid) $userIds[(int)$uid] = true;
        }
        if ($row['type'] === 'chat_package' && !empty($row['reference']) && strncmp($row['reference'], 'session:', 8) === 0) {
          $sessionIds[] = (int)substr($row['reference'], 8);
        }
      }

      // Resolve chat session parties to get other user IDs
      $sessionMap = [];
      if (!empty($sessionIds)) {
        $ph = implode(',', array_fill(0, count($sessionIds), '?'));
        $sst = $pdo->prepare("SELECT id, buyer_id, seller_id FROM chat_time_sessions WHERE id IN ($ph)");
        $sst->execute($sessionIds);
        foreach ($sst->fetchAll(PDO::FETCH_ASSOC) as $s) {
          $sessionMap[(int)$s['id']] = $s;
          $userIds[(int)$s['buyer_id']] = true;
          $userIds[(int)$s['seller_id']] = true;
        }
      }

      // Batch-fetch display names for all referenced user IDs
      $nameMap = [];
      if (!empty($userIds)) {
        $ids = array_keys($userIds);
        $ph = implode(',', array_fill(0, count($ids), '?'));
        $ust = $pdo->prepare("SELECT id, COALESCE(NULLIF(TRIM(name),''), username, CONCAT('user ',id)) AS display_name FROM users WHERE id IN ($ph)");
        $ust->execute($ids);
        foreach ($ust->fetchAll(PDO::FETCH_ASSOC) as $u) {
          $nameMap[(int)$u['id']] = $u['display_name'];
        }
      }

      // Enrich each row with real names
      foreach ($rows as &$row) {
        // Replace "user {id}" patterns in any note with the real display name
        if (!empty($row['note'])) {
          $row['note'] = preg_replace_callback('/\buser (\d+)\b/i', function($m) use ($nameMap) {
            $uid = (int)$m[1];
            return $nameMap[$uid] ?? $m[0];
          }, (string)$row['note']);
        }
        // Enrich chat_package note with the other party's name
        if ($row['type'] === 'chat_package' && !empty($row['reference']) && strncmp($row['reference'], 'session:', 8) === 0) {
          $sid = (int)substr($row['reference'], 8);
          if (isset($sessionMap[$sid])) {
            $s = $sessionMap[$sid];
            $otherId = ((int)$s['buyer_id'] === $userId) ? (int)$s['seller_id'] : (int)$s['buyer_id'];
            $otherName = $nameMap[$otherId] ?? ('user ' . $otherId);
            if ($row['direction'] === 'debit') {
              $row['note'] = preg_replace('/(\d+-min) chat package/', '$1 chat with ' . $otherName, (string)$row['note']);
            } else {
              $row['note'] = preg_replace('/(\d+-min) chat package(\s*\(sold\))?/', '$1 chat with ' . $otherName . ' (sold)', (string)$row['note']);
            }
          }
        }
        // Parse deposit_id from reference field (format: 'qr:{deposit_id}')
        if (!empty($row['reference']) && strncmp($row['reference'], 'qr:', 3) === 0) {
          $row['deposit_id'] = (int)substr($row['reference'], 3);
        }
      }
      unset($row);
      out_json(200, ['status'=>'success','transactions'=>$rows]);
    }
    ...
    // âœ… NEW: Deposit methods (admin QR wallets)
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // âœ… NEW: Create QR deposit (returns QR + deposit_id)
    ...
    // âœ… NEW: Check QR payment (1st click fail, 2nd reviewing)
    ...
    // ============================
    // ✅ NEW: Upload deposit proof screenshot
    // ============================
    if ($action === 'upload_deposit_proof') {
      $deposit_id = (int)($_POST['deposit_id'] ?? 0);
      if ($deposit_id <= 0) out_json(400, ['status'=>'error','message'=>'deposit_id required']);
      ...
      if ($dep['status'] !== 'initiated' && $dep['status'] !== 'reviewing') {
          out_json(400, ['status'=>'error','message'=>'Deposit already processed']);
      }

      if (empty($_FILES['proof']) || !is_uploaded_file($_FILES['proof']['tmp_name'])) {
          out_json(400, ['status'=>'error','message'=>'No image uploaded']);
      }
      ...
      if (!move_uploaded_file($_FILES['proof']['tmp_name'], $dir.$name)) {
          out_json(500, ['status'=>'error','message'=>'Failed to save image on server']);
      }

      $proofUrl = base_url($config)."/api/v1/uploads/wallet/".$name;

      $pdo->prepare("UPDATE wallet_deposits SET proof_url=?, status='reviewing', updated_at=NOW() WHERE id=?")->execute([$proofUrl, $deposit_id]);

      out_json(200, [
        'status' => 'success',
        'message' => 'Screenshot uploaded. Payment under review.'
      ]);
    }
    ...
    if ($action === 'request_deposit' || $action === 'request_withdraw') {
      ...
      if ($reqType === 'withdraw' && $coins < (int)$s['min_withdraw_coins']) { out_json(400, ['status'=>'error','message'=>'Below minimum withdraw coins']); }
      if ($reqType === 'deposit' && $coins < (int)$s['min_deposit_coins']) { out_json(400, ['status'=>'error','message'=>'Below minimum deposit coins']); }
      $currencyCode = (string)($s['currency_code'] ?? 'NPR');
      $coinsPer = (float)($s['coins_per_currency'] ?? 1);
      if ($coinsPer <= 0) $coinsPer = 1;
      $currencyAmount = $coins / $coinsPer;
      ...
      if (!empty($_FILES['proof']) && is_uploaded_file($_FILES['proof']['tmp_name'])) {
        ...
        $ext = $ext ? '.'.preg_replace('/[^a-zA-Z0-9]/', '', $ext) : '';
        $name = uniqid('proof_', true).$ext;
        if (move_uploaded_file($_FILES['proof']['tmp_name'], $dir.$name)) { $proofUrl = base_url($config).'/api/v1/uploads/wallet/'.$name; }
      }
      $st = $pdo->prepare("INSERT INTO wallet_requests (user_id, req_type, coins, currency_amount, currency_code, method, proof_url, status) VALUES (?,?,?,?,?,?,?, 'pending')");
      $st->execute([$userId, $reqType, $coins, $currencyAmount, $currencyCode, ($method !== '' ? $method : null), $proofUrl]);
      $reqId = (int)$pdo->lastInsertId();
      add_tx($pdo, $userId, $reqType, ($reqType==='deposit' ? 'credit' : 'debit'), $coins, $currencyAmount, $currencyCode, 'pending', 'req:'.$reqId, ($note !== '' ? $note : null));

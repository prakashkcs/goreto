...
// db_connect.php lives in the same directory as this file (api/v1/)
...
try {
  ...
  $count = $pdo->query("SELECT COUNT(*) FROM legal_pages")->fetchColumn();
  if ((int) $count === 0) {
    $ins = $pdo->prepare("INSERT IGNORE INTO legal_pages (page_key, title, content) VALUES (?,?,?)");
    $ins->execute(['terms', 'Terms & Conditions', default_terms()]);
    $ins->execute(['privacy', 'Privacy Policy', default_privacy()]);
  }
  // Ensure newer policy pages exist even on already-seeded installs
  $ensureLegal = $pdo->prepare("INSERT IGNORE INTO legal_pages (page_key, title, content) VALUES (?,?,?)");
  $ensureLegal->execute(['content_policy', 'Community & Content Policy', default_content_policy()]);
  $ensureLegal->execute(['child_safety', 'Child Safety Standards', default_child_safety()]);
} catch (Throwable $e) {
...
// GET — public fetch (no auth required)
if ($_SERVER['REQUEST_METHOD'] === 'GET' && in_array($action, ['terms', 'privacy', 'content_policy', 'child_safety'])) {
  $stmt = $pdo->prepare("SELECT title, content, updated_at FROM legal_pages WHERE page_key = ?");
  $stmt->execute([$action]);
  $row = $stmt->fetch(PDO::FETCH_ASSOC);
  if (!$row) {
    // Return default content even if DB row missing
    $defs = [
      'terms' => ['Terms & Conditions', default_terms()],
      'privacy' => ['Privacy Policy', default_privacy()],
      'content_policy' => ['Community & Content Policy', default_content_policy()],
      'child_safety' => ['Child Safety Standards', default_child_safety()],
    ];
    $d = $defs[$action] ?? $defs['terms'];
    $row = ['title' => $d[0], 'content' => $d[1], 'updated_at' => date('Y-m-d H:i:s')];
  }
  json_out(200, ['status' => 'success', 'page' => $row]);
}

// POST — admin update (requires admin session via token header)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'update') {
  ...
  if (!in_array($key, ['terms', 'privacy', 'content_policy', 'child_safety']) || !$title || !$content) {
...
<p>Eklo offers optional subscription plans that unlock premium features. Subscriptions are billed on a recurring basis (weekly, monthly, or as specified). Subscriptions auto-renew unless cancelled at least 24 hours before the renewal date. You can manage or cancel subscriptions through your device's app store (Google Play Store / Apple App Store). Refunds for subscriptions are governed by the respective app store's refund policy. We do not offer direct refunds for subscription fees except as required by law.</p>
...
<p>All App content, design, logos, trademarks, software, and features (excluding user-generated content) are the exclusive property of Eklo and are protected under Nepal's Copyright Act 2059 and international intellectual property laws. You may not copy, modify, distribute, sell, or create derivative works from our proprietary content without express written permission. If you believe your copyright has been infringed, contact us at <strong>legal@eklo.app</strong> with details of the alleged infringement.</p>
...
<p>THE APP IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EXPRESS OR IMPLIED. WE DO NOT GUARANTEE UNINTERRUPTED, ERROR-FREE, OR VIRUS-FREE SERVICE. WE ARE NOT RESPONSIBLE FOR ANY INTERACTIONS, MEETINGS, OR TRANSACTIONS BETWEEN USERS. TO THE MAXIMUM EXTENT PERMITTED BY NEPALI LAW, EKLO SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES ARISING FROM YOUR USE OF THE APP.</p>
...
<ul>
  <li>Email: <strong>support@eklo.app</strong></li>
  <li>Legal: <strong>legal@eklo.app</strong></li>
...
<p>Effective Date: April 27, 2025 | Last Updated: April 27, 2025</p>
<p><strong>Eklo</strong> ("we", "us", or "our") is committed to protecting your personal information. This Privacy Policy explains how we collect, use, store, share, and protect your data when you use our App. We comply with Nepal's <strong>Individual Privacy Act 2075 (Byaktigat Gupta ta Sambandhi Ain 2075)</strong> and applicable international data protection standards.</p>
...
<p>Eklo is strictly for users aged <strong>18 and older</strong>. We do not knowingly collect personal information from anyone under 18. If we become aware that a minor has created an account, we will immediately delete all their data and terminate the account. If you believe a minor is using the App, please report it to <strong>support@eklo.app</strong> immediately.</p>
...
<p>To exercise any of these rights, contact us at <strong>privacy@eklo.app</strong>. We will respond within 30 days.</p>
...
<ul>
  <li>Email: <strong>privacy@eklo.app</strong></li>
  <li>Support: <strong>support@eklo.app</strong></li>
  <li>Address: Kathmandu, Nepal</li>
</ul>
...
// ââ Community & Content Policy ââ
function default_content_policy(): string
{
  return <<<HTML
<h2>Community &amp; Content Policy</h2>
<p>This Policy governs all content and conduct on Eklo/Goreto ("the App"). It applies to posts, reels, photos, audio, live streams, comments, messages, profiles and any other user-generated content. By using the App you agree to follow this Policy. We operate in accordance with Nepal's Electronic Transactions Act 2063, the Social Media Directives 2080 (2023), and the policies of the Apple App Store and Google Play.</p>

<h3>1. Zero Tolerance: Child Safety</h3>
<p>Child sexual abuse and exploitation (CSAE) material and any content that sexualises minors are strictly prohibited and will be removed immediately. Such material is reported to the Nepal Police Cyber Bureau and relevant authorities. See our separate <strong>Child Safety Standards</strong> for full details.</p>

<h3>2. Prohibited Content</h3>
<p>You may not post, share, stream, or send:</p>
<ul>
  <li>Sexually explicit material, pornography, or nudity</li>
  <li>Content that sexualises, endangers, or exploits minors (CSAE)</li>
  <li>Hate speech or content inciting hatred against any caste, ethnicity, religion, gender, or nationality (prohibited under ETA 2063 Section 47)</li>
  <li>Graphic violence, gore, or content that promotes self-harm, suicide, or terrorism</li>
  <li>Harassment, bullying, threats, doxxing, or stalking</li>
  <li>Non-consensual intimate images or "revenge porn"</li>
  <li>Deepfakes or manipulated media intended to deceive or harm</li>
  <li>Impersonation, fake accounts, or identity theft</li>
  <li>Spam, scams, fraud, or misleading financial schemes</li>
  <li>Content that is illegal under the laws of Nepal or that violates others' intellectual property</li>
</ul>

<h3>3. Moderation &amp; User Controls</h3>
<p>We use a combination of automated screening and human review. Every user can:</p>
<ul>
  <li><strong>Report</strong> any post, comment, sound, profile, or live stream from within the App</li>
  <li><strong>Block</strong> any user to stop all contact and visibility</li>
  <li><strong>Mute</strong> users to suppress their notifications</li>
</ul>

<h3>4. 24-Hour Takedown</h3>
<p>In line with Nepal's Social Media Directives 2080 (2023), we act on valid reports of unlawful or objectionable content and remove confirmed violations within <strong>24 hours</strong>. Serious violations (CSAE, credible threats) are actioned immediately.</p>

<h3>5. Enforcement</h3>
<p>Violations may result in content removal, feature restrictions, temporary suspension, or permanent account termination and device ban, depending on severity. We may preserve and disclose information to law enforcement where required by law.</p>

<h3>6. Reporting &amp; Contact</h3>
<p>To report content or appeal a decision, use the in-app report tools or contact <strong>support@eklo.app</strong>. Child-safety concerns: <strong>childsafety@eklo.app</strong>.</p>
HTML;
}

// ââ Child Safety Standards ââ
function default_child_safety(): string
{
  return <<<HTML
<h2>Child Safety Standards</h2>
<p>Eklo/Goreto ("the App") has a <strong>zero-tolerance policy</strong> toward child sexual abuse and exploitation (CSAE). These Standards explain how we prohibit, detect, and respond to CSAE, in compliance with Google Play's Child Safety Standards policy, Apple App Store guidelines, and the laws of Nepal (including the National Penal Code 2074 and the Children's Act 2075).</p>

<h3>1. Minimum Age: 18+</h3>
<p>The App is strictly for adults aged <strong>18 and over</strong>. We collect date of birth at registration and block anyone under 18. Accounts found to belong to minors are terminated.</p>

<h3>2. Prohibited Conduct</h3>
<ul>
  <li>Any CSAE material — images, video, audio, text, or links</li>
  <li>Sexualisation of minors, including grooming, solicitation, or sexual commentary</li>
  <li>Attempting to arrange or facilitate contact with a minor for sexual purposes</li>
  <li>Sharing or requesting intimate content involving anyone under 18</li>
</ul>

<h3>3. How We Detect &amp; Prevent</h3>
<ul>
  <li>Age-gated registration (18+) with date-of-birth verification</li>
  <li>Automated and human content moderation</li>
  <li>A dedicated in-app report category for "Child sexual abuse / exploitation (CSAE)" on posts, profiles, sounds, and users</li>
  <li>Immediate removal and account termination on confirmed violations</li>
</ul>

<h3>4. Reporting CSAE</h3>
<p>Anyone can report suspected CSAE instantly using the report option in the App (select "Child sexual abuse / exploitation"). Reports are prioritised and escalated for immediate review.</p>

<h3>5. Cooperation with Authorities</h3>
<p>We report confirmed CSAE to the <strong>Nepal Police Cyber Bureau</strong> and cooperate with lawful requests from law enforcement. Where applicable we also report to international bodies such as the National Center for Missing &amp; Exploited Children (NCMEC).</p>

<h3>6. Child Safety Point of Contact</h3>
<p>Our designated Child Safety contact can be reached at <strong>childsafety@eklo.app</strong>. We respond to child-safety reports as a top priority, within 24 hours.</p>
HTML;
}

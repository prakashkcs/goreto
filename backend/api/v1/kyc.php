...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'status' || $action === '') {
      // Prefer user_kyc.basic_status (the canonical source updated by
      // submit_kyc + admin review). Fall back to users.kyc_status only if
      // user_kyc has no row yet — the legacy column is not updated on submit
      // so reading it makes the app show 'not submitted' right after a
      // successful submit.
      $st = $pdo->prepare("
          SELECT COALESCE(NULLIF(uk.basic_status,''), NULLIF(u.kyc_status,''), 'none') AS basic_status,
                 COALESCE(NULLIF(uk.full_status,''),  NULLIF(u.kyc_status,''), 'none') AS full_status
          FROM users u
      ...
      $st->execute([$userId]);
      $row = $st->fetch(PDO::FETCH_ASSOC);
      $status = $row['basic_status'] ?? 'none';
      if ($status === 'approved') $status = 'verified';

      // The admin_note column does not exist on kyc_verifications, so we default to empty.
      $note = '';

      // basic_status and full_status are kept distinct so the UI can
      // surface them independently — most callers only look at basic.
      $fullStatus = $row['full_status'] ?? 'none';
      if ($fullStatus === 'approved') $fullStatus = 'verified';
      $resp = [
          'user_id' => $userId,
          'basic_status' => $status,
      ...
      out_json(200, ['status'=>'success','kyc'=>$resp]);
    }

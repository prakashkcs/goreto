  ...
  if ($id > 0 && $action === 'wallet_adjust') {
    ...
    if ($coins > 0) {
      ...
      if ($mode === 'subtract') {
        ...
        try { send_notif($pdo, $id, 'wallet_reject', 'Balance Adjusted', "Admin removed {$coins} coins from your wallet." . ($note ? " Note: {$note}" : '')); } catch(Throwable $_){}
      } else {
        $newBal = $w['balance_coins'] + $coins;
        $pdo->prepare("UPDATE user_wallets SET balance_coins=?,updated_at=NOW() WHERE user_id=?")->execute([$newBal, $id]);
        log_wallet_u($pdo, $id, 'admin_adjust', 'credit', $coins, $note ?: 'Admin added coins');
        try { send_notif($pdo, $id, 'wallet_accept', 'Balance Added', "Admin added {$coins} coins to your wallet." . ($note ? " Note: {$note}" : '')); } catch(Throwable $_){}
      }
    }
    header("Location: users.php?edit=$id");
    exit;
...
$colLastSeen = $pick(['last_seen', 'last_active', 'last_login']);
...
$sel = implode(',', array_filter([
  ...
  "subscription_status",
  ($colLastSeen ? "$colLastSeen AS last_seen" : "NULL AS last_seen"),
]));

$users = [];
$inactiveFilter = in_array($_GET['inactive'] ?? '', ['7d','30d','1y'], true) ? ($_GET['inactive']) : '';

// Handle bulk delete of inactive users
if (isset($_GET['bulk_delete_inactive']) && !empty($inactiveFilter) && $colId) {
  $dayMap2 = ['7d'=>7,'30d'=>30,'1y'=>365];
  $days2 = $dayMap2[$inactiveFilter];
  try {
    $idsRes = $pdo->query("SELECT id FROM users WHERE (last_seen IS NULL OR last_seen < NOW() - INTERVAL {$days2} DAY) AND is_banned=0 ORDER BY id ASC LIMIT 5000");
    $ids = array_column($idsRes->fetchAll(), 'id');
    foreach ($ids as $delId) {
      foreach (['post_likes','post_comments','posts','stories','collections','user_sessions','wallet_transactions','user_wallets'] as $t)
        try { $pdo->prepare("DELETE FROM $t WHERE user_id=?")->execute([$delId]); } catch(Throwable $ex){}
      try { $pdo->prepare("DELETE FROM users WHERE id=?")->execute([$delId]); } catch(Throwable $ex){}
    }
    header("Location: users.php?inactive={$inactiveFilter}&bulk_done=".count($ids));
    exit;
  } catch (Throwable $e) {}
}
...
if ($colId) {
  try {
    $dayMap = ['7d'=>7,'30d'=>30,'1y'=>365];
    if ($inactiveFilter && isset($dayMap[$inactiveFilter])) {
      $days = $dayMap[$inactiveFilter];
      $users = $pdo->query("SELECT $sel FROM users WHERE (last_seen IS NULL OR last_seen < NOW() - INTERVAL {$days} DAY) ORDER BY last_seen ASC LIMIT 2000")->fetchAll();
    } else {
...
<div class="section">
  <div class="head" style="flex-wrap:wrap;gap:10px">
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      <span style="color:#9fb4d1;font-size:13px">Inactive filter:</span>
      <a class="btn <?= $inactiveFilter==='' ? 'ok' : '' ?>" href="users.php">All</a>
      <a class="btn <?= $inactiveFilter==='7d' ? 'ok' : '' ?>" href="?inactive=7d">No activity 7 days</a>
      <a class="btn <?= $inactiveFilter==='30d' ? 'ok' : '' ?>" href="?inactive=30d">30 days</a>
      <a class="btn <?= $inactiveFilter==='1y' ? 'ok' : '' ?>" href="?inactive=1y">1 year</a>
      <?php if ($inactiveFilter): ?>
        <a class="btn danger"
           href="?inactive=<?= $inactiveFilter ?>&bulk_delete_inactive=1"
           onclick="return confirm('Permanently delete ALL ' + <?= count($users) ?> + ' inactive users shown? This cannot be undone.')">
           Delete All (<?= count($users) ?>)
        </a>
      <?php endif; ?>
    </div>
    <div class="search">
  ...
  <?php if (!empty($_GET['bulk_done'])): ?>
    <div style="padding:10px 16px"><span class="badge ok">Deleted <?= (int)$_GET['bulk_done'] ?> inactive users successfully.</span></div>
  <?php endif; ?>
  <div class="body">
    <div class="table-wrap">
      <table id="tableUsers">
        ...
        <tbody>
          <?php foreach ($users as $u):
            ...
            <tr>
              ...
              <td><small style="color:#a0b4cc"><?= !empty($u['last_seen']) ? h(substr($u['last_seen'], 0, 16)) : '<span style="color:#f87171">never</span>' ?></small></td>

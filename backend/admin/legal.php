...
// Deploy config
// FTP removed — same-server deploy
...
// api_legal.php now lives in api/v1/ on this server — no separate local deploy needed.
$LOCAL_API_LEGAL = __DIR__ . '/../api/v1/api_legal.php';
...
// ── FTP deploy helper ────────────────────────────────────────────────────────
// ── Deploy helper (no FTP needed — file is already on this server) ────────
function ftp_deploy_file(string $localFile, array $remotePaths): array
{
    if (!file_exists($localFile)) {
        return ['ok' => false, 'msg' => "File not found: $localFile"];
    }
    touch($localFile);
    if (function_exists('opcache_invalidate')) opcache_invalidate($localFile, true);
    return ['ok' => true, 'msg' => "API file is live at $localFile (same server — no FTP needed)."];
}

                ...
                <?php foreach ($rows as $r): ?>
                    ...
                    <div class="audio-report-card">
                        ...
                        <form method="post" class="audio-form">
                            ...
                            <button class="btn warn" type="button" onclick="ajaxSoundAction('remove_sound',<?= (int)$r['id'] ?>,<?= (int)$r['post_id'] ?>,this)">Remove Sound</button>
                            <button class="btn danger" type="button" onclick="ajaxSoundAction('delete_post',<?= (int)$r['id'] ?>,<?= (int)$r['post_id'] ?>,this)">Delete Post</button>
                            <button class="btn danger" type="button" onclick="ajaxSoundAction('delete_report',<?= (int)$r['id'] ?>,<?= (int)$r['post_id'] ?>,this)">Delete Report</button>
                        </form>
                    </div>
                <?php endforeach; ?>
            </div>
...
<script>
function ajaxSoundAction(action, reportId, postId, btn) {
  var msgs = {
    remove_sound: 'Remove sound from post #'+postId+'? This clears the audio but keeps the post.',
    delete_post:  'Permanently DELETE post #'+postId+'? This cannot be undone.',
    delete_report:'Delete audio report #'+reportId+'?'
  };
  if (!confirm(msgs[action] || 'Confirm action?')) return;
  btn.disabled = true;
  var fd = new FormData();
  ...
  fetch('?ajax='+action, {method:'POST', body:fd})
    ...
    .then(function(data){
      if (data.status === 'success') {
        var card = btn.closest('.audio-report-card');
        if (action === 'delete_report') {
          card.style.transition = 'opacity .3s';
          card.style.opacity = '0';
          setTimeout(function(){card.remove();}, 320);
        } else if (action === 'remove_sound') {
          btn.textContent = '✓ Sound Removed';
          btn.style.opacity = '0.6';
        } else if (action === 'delete_post') {
          card.style.transition = 'opacity .3s';
          card.style.opacity = '0.3';
          btn.textContent = '✓ Post Deleted';
          btn.style.opacity = '0.6';
        }
      } else {
        btn.disabled = false;

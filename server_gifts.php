...
function ensure_tables(PDO $pdo): void
{
  ...
  // Seed love-themed gifts if table is empty
  try {
    $cnt = (int)$pdo->query("SELECT COUNT(*) FROM gifts")->fetchColumn();
    if ($cnt === 0) {
      $ins = $pdo->prepare("INSERT INTO gifts (name, coin_price, is_active) VALUES (?, ?, 1)");
      foreach ([
        ['Rose',          10],
        ['Kiss',          25],
        ['Sweet Heart',   50],
...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'list' || $action === '') {
      ...
      try {
        $rows = $pdo->query("
          ...
          ORDER BY coin_price DESC, id DESC
    ...
    // Public gift shelf for a user's profile — no auth needed
    if ($action === 'received') {
      $targetId = (int)($_GET['user_id'] ?? 0);
      if ($targetId <= 0) out_json(400, ['status' => false, 'message' => 'user_id required']);
      ...
      try {
        $st = $pdo->prepare("
          ...
          JOIN gifts g ON g.id = ug.gift_id
          WHERE ug.user_id = ? AND ug.qty > 0
          ORDER BY g.coin_price DESC, ug.qty DESC, ug.updated_at DESC
          LIMIT 50
        ");
        $st->execute([$targetId]);
        $items = $st->fetchAll(PDO::FETCH_ASSOC) ?: [];

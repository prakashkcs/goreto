...
try {
  ...
  // Migrate: add proof_url to existing wallet_deposits tables
  try {
    $cols = array_column($pdo->query("SHOW COLUMNS FROM wallet_deposits")->fetchAll(PDO::FETCH_ASSOC), 'Field');
    if (!in_array('proof_url', $cols, true)) {
      $pdo->exec("ALTER TABLE wallet_deposits ADD COLUMN proof_url VARCHAR(255) NULL AFTER click_count");
    }
  } catch (Throwable $_) {}

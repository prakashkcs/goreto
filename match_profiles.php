...
if ($action === 'get_my_profile' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($profile) {
        ...
        // Fetch public partner only if BOTH sides have sent accepted proposals
        // (true mutual connection: A→B accepted AND B→A accepted)
        ...
        WHERE (p.sender_id = ? OR p.receiver_id = ?) AND p.status = 'accepted' AND p.show_on_profile = 1
          AND EXISTS (
              ...
              WHERE p2.sender_id = p.receiver_id AND p2.receiver_id = p.sender_id
                AND p2.status = 'accepted' AND p2.show_on_profile = 1
          )
        LIMIT 1
    ");
...
if ($action === 'update_location' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (!in_array('lat', $cols))
            $pdo->exec("ALTER TABLE match_profiles ADD COLUMN lat DOUBLE NULL");
        if (!in_array('lng', $cols))
            $pdo->exec("ALTER TABLE match_profiles ADD COLUMN lng DOUBLE NULL");
        if (!in_array('last_location_update', $cols))
            $pdo->exec("ALTER TABLE match_profiles ADD COLUMN last_location_update DATETIME NULL");
        if (!in_array('last_nearby_notif', $cols))
            $pdo->exec("ALTER TABLE match_profiles ADD COLUMN last_nearby_notif DATETIME NULL");
    ...
    $nearbyStmt = $pdo->prepare("
        SELECT mp.user_id, mp.lat, mp.lng, mp.last_nearby_notif,
               ...
               COALESCE(u.privacy_share_distance, 1) AS share_dist,
               (6371 * acos(
    ...
    if (!empty($nearbyUsers)) {
        ...
        // Get current user name + privacy setting for the notification
        $meStmt = $pdo->prepare("SELECT name, username, COALESCE(privacy_share_distance, 1) AS share_dist FROM users WHERE id = ?");
        $meStmt->execute([$userId]);
        $me = $meStmt->fetch(PDO::FETCH_ASSOC);
        $myName = $me['name'] ?: $me['username'] ?: 'Someone';
        $myShareDist = (int)($me['share_dist'] ?? 1);
        ...
        foreach ($nearbyUsers as $nearby) {
            ...
            // Send data-only FCM so the background handler fires reliably on OEM devices
            // (Xiaomi, Oppo, etc. suppress notification+data auto-display).
            // The Flutter background handler calls showTray() which shows it manually.
            $title = '📍 Someone is nearby!';
            $body = $myShareDist ? "$myName is about {$distMeters}m away from you right now." : "$myName is nearby!";
            $nearbyExtra = ['sender_distance' => (string)$distMeters];
            send_app_notification($pdo, $nearbyUid, $userId, 'nearby', $title, $body, null, true, $nearbyExtra);

            // Also notify the current user about the nearby person
            $theirName = $nearby['name'] ?: $nearby['username'] ?: 'Someone';
            $theirShareDist = (int)($nearby['share_dist'] ?? 1);
            $title2 = '📍 Someone is nearby!';
            $body2 = $theirShareDist ? "$theirName is about {$distMeters}m away from you right now." : "$theirName is nearby!";
            send_app_notification($pdo, $userId, $nearbyUid, 'nearby', $title2, $body2, null, true, $nearbyExtra);
...
if ($action === 'nearby' && $_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Also resolve the authenticated user from the Bearer token and exclude them.
    // This prevents the caller's own profile from appearing even if user_id param is wrong.
    $authUserId = 0;
    $nbHeaders = function_exists('getallheaders') ? getallheaders() : [];
    $nbAuth = $nbHeaders['Authorization'] ?? $nbHeaders['authorization'] ?? $_SERVER['HTTP_AUTHORIZATION'] ?? '';
    $nbToken = trim($nbAuth);
    if (stripos($nbToken, 'Bearer ') === 0) $nbToken = trim(substr($nbToken, 7));
    if ($nbToken !== '') {
        $stNb = $pdo->prepare("SELECT id FROM users WHERE api_token = ? LIMIT 1");
        $stNb->execute([$nbToken]);
        $authUserId = (int) $stNb->fetchColumn();
        if (!$authUserId) {
            $stNb2 = $pdo->prepare("SELECT user_id FROM user_auth_tokens WHERE token = ? AND revoked_at IS NULL LIMIT 1");
            $stNb2->execute([$nbToken]);
            $authUserId = (int) $stNb2->fetchColumn();
        }
        // If token resolves a different user than the param, use auth user as primary
        if ($authUserId > 0 && $userId <= 0) $userId = $authUserId;
    }

    // Ensure rating column exists on match_profiles
    try {
        $mpCols = [];
        foreach ($pdo->query("SHOW COLUMNS FROM match_profiles")->fetchAll(PDO::FETCH_ASSOC) as $r) {
            $mpCols[] = $r['Field'];
        }
        if (!in_array('rating', $mpCols)) {
            $pdo->exec("ALTER TABLE match_profiles ADD COLUMN rating DOUBLE NOT NULL DEFAULT 0");
        }
    } catch (Throwable $e) { /* ignore */
    }
    ...
    // Use COALESCE to fall back to users.latitude/longitude when match_profiles has no coords
    // Only show partner if BOTH sides have show_on_profile = 1 (mutual consent)
    $partnerSubquery = "
        (SELECT JSON_OBJECT(
            'partner_id', u2.id,
            'partner_name', u2.name,
        ...
        JOIN users u2 ON u2.id = CASE WHEN p.sender_id = mp.user_id THEN p.receiver_id ELSE p.sender_id END
        LEFT JOIN match_profiles m2 ON m2.user_id = u2.id
        WHERE (p.sender_id = mp.user_id OR p.receiver_id = mp.user_id)
          AND p.status = 'accepted' AND p.show_on_profile = 1
          AND EXISTS (
              SELECT 1 FROM proposals p2
              WHERE p2.sender_id = p.receiver_id AND p2.receiver_id = p.sender_id
                AND p2.status = 'accepted' AND p2.show_on_profile = 1
          )
        LIMIT 1) as public_partner_json";

    if ($hasLocation) {
        // Distance using COALESCE(match lat, users lat)
        $sql = "
            SELECT mp.*, u.name, u.username, u.profile_pic as user_avatar,
                   COALESCE(mp.lat, u.latitude) as eff_lat,
                   COALESCE(mp.lng, u.longitude) as eff_lng,
                   (6371 * acos(GREATEST(-1, LEAST(1,
                       cos(radians(?)) * cos(radians(COALESCE(mp.lat, u.latitude))) *
                       cos(radians(COALESCE(mp.lng, u.longitude)) - radians(?)) +
                       sin(radians(?)) * sin(radians(COALESCE(mp.lat, u.latitude)))
                   )))) AS distance_km,
                   COALESCE(u.privacy_share_distance, 1) AS _share_dist,
                   $partnerSubquery
            FROM match_profiles mp
            JOIN users u ON u.id = mp.user_id
            WHERE mp.user_id != ?
              AND ($authUserId <= 0 OR mp.user_id != $authUserId)
              AND mp.is_visible = 1
              AND COALESCE(u.privacy_nearby_visible, 1) = 1
              AND (mp.lat IS NOT NULL OR u.latitude IS NOT NULL)";
    ...
    // Exclude accepted proposals and blocked users
    $sql .= "
          AND NOT EXISTS (
              SELECT 1 FROM proposals p2
              WHERE ((p2.sender_id = mp.user_id AND p2.receiver_id = ?)
    ...
    if ($hasLocation) {
        $sql .= " HAVING distance_km <= 100 ORDER BY distance_km ASC";
    } else {
    ...
    foreach ($profiles as &$p) {
        ...
        // Respect privacy_share_distance: hide distance if user opted out
        if (isset($p['_share_dist']) && (int)$p['_share_dist'] === 0) {
            $p['distance_km'] = null;
        } elseif (isset($p['distance_km']) && $p['distance_km'] !== null) {

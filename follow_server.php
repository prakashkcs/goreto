...
function counts(PDO $pdo, int $userId): array
{
  ...
  $blockFilter = "";
  if ($me > 0) {
    // Exclude users blocked by or who blocked the viewer
    $blockFilter = " AND follower_id NOT IN (SELECT blocked_id FROM user_blocks WHERE blocker_id = $me) 
                     AND follower_id NOT IN (SELECT blocker_id FROM user_blocks WHERE blocked_id = $me)
                     AND following_id NOT IN (SELECT blocked_id FROM user_blocks WHERE blocker_id = $me) 
                     AND following_id NOT IN (SELECT blocker_id FROM user_blocks WHERE blocked_id = $me)";
  }

  $followers = (int)$pdo->query("SELECT COUNT(*) FROM follows WHERE following_id = " . intval($userId) . $blockFilter)->fetchColumn();
  $following = (int)$pdo->query("SELECT COUNT(*) FROM follows WHERE follower_id = " . intval($userId) . $blockFilter)->fetchColumn();
  
  $postBlockFilter = "";
  if ($me > 0) {
      $postBlockFilter = " AND user_id NOT IN (SELECT blocked_id FROM user_blocks WHERE blocker_id = $me) 
                           AND user_id NOT IN (SELECT blocker_id FROM user_blocks WHERE blocked_id = $me)";
  }
  
  $posts = 0;
  try {
    $posts = (int)$pdo->query("SELECT COUNT(*) FROM posts WHERE user_id = " . intval($userId) . $postBlockFilter)->fetchColumn();
  }
  catch (Throwable $e) {
  }

...
try {
    ...
    elseif ($action === 'toggle_public_proposal') {
        $proposalId = (int)($_POST['proposal_id'] ?? 0);
        $isPublicRaw = $_POST['is_public'] ?? 'not set';
        $isPublic = ($isPublicRaw === 'true' || $isPublicRaw === '1' || $isPublicRaw === true || $isPublicRaw === 1) ? 1 : 0;
        
        // Debug log
        file_put_contents('debug_toggle.log', date('[Y-m-d H:i:s] ') . "Action: $action, ID: $proposalId, Raw: " . json_encode($isPublicRaw) . ", Final: $isPublic, User: $userId\n", FILE_APPEND);
        ...
        if ($stmt->rowCount() > 0) {
            ...
            // Check if proposal exists at all
            $chk = $pdo->prepare("SELECT status FROM proposals WHERE id = ?");
            $chk->execute([$proposalId]);
            $status = $chk->fetchColumn();
            echo json_encode(['status' => 'error', 'message' => "Update failed. Proposal status: " . ($status ?: 'not found')]);
        }
    }
    // ════════════════════════════════════════════════════════
    //  DISCONNECT
